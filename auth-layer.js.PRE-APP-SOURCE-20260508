...
(function () {
  ...
  function syncFieldTechName(user) {
    ...
    _doSync();
    // Retry after React has mounted to ensure the storage event is received
  ...
  async function bootstrap() {
    ...
    if (token) {
      try {
        ...
        console.warn('[WC Auth] bootstrap fetch failed:', e);
        // Token invalid/expired — clear it and show login
        clearToken();
      }
    }

    // Always show something — never leave a dark screen
    const root2 = document.getElementById('root');
    if (root2 && root2.style.display === 'none') {
      renderLogin();
    }
  }
